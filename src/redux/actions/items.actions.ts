...
export const createItemThunk = createAsyncThunk<Item, { institutionId: string; name: string; article: string }>(
  ...
  async (params, { dispatch, fulfillWithValue, rejectWithValue }) => {
    try {
      const res = await api.post<any, { data: Item | BackendError | undefined }>("/item/create", {
...
export const fetchItemThunk = createAsyncThunk<any, { id: string }>("item/fetch", async (params, { dispatch, rejectWithValue, fulfillWithValue }) => {
  try {
    const res = await api.get<any, { data: Item | BackendError | undefined }>("/item/", { params: { id: params.id } });
...
export const searchItemThunk = createAsyncThunk<Item[], { institution: string; article: string; take: number; skip: number; id?: string }>(
  "item/search",
  async (params, { rejectWithValue, fulfillWithValue }) => {
    try {
      const res = await api.get<any, { data: { items: Item[] } | BackendError | undefined }>(`/item`, { params: { ...params } });
